		...
		case $choice in
			1)
				...
				while true; do
					...
					if ss -tuln | grep -q ":$app_port "; then
						echo -e "${gl_hong}錯誤:${gl_bai}端口 $app_port 已被占用，请更换一个端口"
						send_stats "應用程式連接埠已被佔用"
					else
						local docker_port=$app_port
						break
...
bbrv3() {
		  ...
		  xanmod_add_repo() {
				...
				if echo "jammy focal bullseye buster" | grep -qw "$os_codename" || [ "$os_codename" = "releases" ]; then
					echo -e "${gl_hong}XanMod 官方已停止對目前系統($os_codename)的 APT 源支持，请升级至 Debian12 / Ubuntu24 或更高版本。${gl_bai}"
					return 1
				fi
...
clamav() {
		  ...
		  while true; do
				...
				echo "------------------------"
				echo -e "${gl_lv}1. 全盤掃描${gl_bai}             ${gl_huang}2. 重要目录扫描 ${gl_bai}            ${gl_kjlan}3. 自訂目錄掃描${gl_bai}"
				echo "------------------------"
				echo "0. 返回上一級選單"
				echo "------------------------"
				read -e -p "請輸入你的選擇:" sub_choice
...
EOF
	...
	add-all-models-from-provider() {
		...
		echo "🔍 正在獲取$provider_name 的所有可用模型..."
...
PY
		...
		# 產生 JSON 並透過 openclaw approvals set --stdin 寫入（優先）
		# 若 CLI 不支持则回退直接写文件
		local json_payload
...
' "$allowlist_json"
	...
	openclaw_permission_menu() {
		...
		while true; do
			...
			echo -e "${gl_kjlan}3.${gl_bai}切換為完全開放模式（${gl_hong}高風險！彻底解除所有宿主机拦截${gl_bai}）"
...
' "$(openclaw_multiagent_agents_json)" "$(openclaw_multiagent_bindings_json)"
	...
	openclaw_multiagent_set_identity() {
		...
		echo "也可以從 IDENTITY.md 自動讀取身分資訊。"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}b.   ${gl_bai}備份全部應用數據${gl_kjlan}r.   ${gl_bai}还原全部应用数据"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}返回主選單"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}3.   ${gl_bai}使用者密碼登入模式${gl_kjlan}4.   ${gl_bai}安装Python指定版本"
	  echo -e "${gl_kjlan}5.   ${gl_bai}开放所有端口                       ${gl_kjlan}6.   ${gl_bai}修改SSH連接埠"
	  ...
	  echo -e "${gl_kjlan}15.  ${gl_bai}系統時區調整${gl_kjlan}16.  ${gl_bai}设置BBR3加速"
	  ...
	  echo -e "${gl_kjlan}35.  ${gl_bai}ssh遠端連線工具${gl_kjlan}36.  ${gl_bai}硬盘分区管理工具"
...
EOF
		  ...
		  66)
			  ...
			  echo -e "9. 自动优化DNS地址${gl_huang}海外: 1.1.1.1 8.8.8.8  国内: 223.5.5.5 ${gl_bai}"
		  	  echo -e "10. 设置网络为${gl_huang}ipv4優先${gl_bai}"
