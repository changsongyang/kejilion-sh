...
certs_status() {
	...
	if [ -f "$file_path" ]; then
		...
		echo "------------------------"
		echo "1. 重新申請 2. 匯入已有憑證 3. 不含憑證改用HTTP存取 0. 退出"
		echo "------------------------"
		read -e -p "請輸入你的選擇:" sub_choice
		case $sub_choice in
	  	  1)
...
fetch_github_ssh_keys() {
	...
	echo "1. 登入 https://github.com/settings/keys"
	...
	echo "https://github.com/您的使用者名稱.keys"
	echo ""
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo -e "ROOT私鑰登入模式${IS_KEY_ENABLED}"
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  111|convertx)
		...
		local docker_url="專案地址: https://github.com/c4illin/ConvertX"
		local docker_use=""
		local docker_passwd=""
		local app_size="2"
		docker_app
	  ...
	  112|lucky)
		...
		local docker_url="專案地址: https://github.com/gdy666/lucky"
...
log_menu() {
	...
	while true; do
		...
		case $choice in
			1)
				send_stats "查看最近日誌"
				read -rp "查看最近多少行日誌？ [預設 100]:" lines
				lines=${lines:-100}
			...
			2)
				send_stats "查看指定服務日誌"
				read -rp "請輸入服務名稱（如 sshd、nginx）:" svc
