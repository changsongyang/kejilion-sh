		...
		1)
			...
			while true; do
				...
				if ss -tuln | grep -q ":$app_port "; then
					echo -e "${gl_hong}错误: ${gl_bai}端口 $app_portすでに占有されています。ポートを変更してください"
					send_stats "应用端口已被占用"
				else
		...
		case $choice in
			1)
				...
				while true; do
					...
					if ss -tuln | grep -q ":$app_port "; then
						echo -e "${gl_hong}错误: ${gl_bai}ポート$app_portすでに占有されています。ポートを変更してください"
						send_stats "应用端口已被占用"
					else
						local docker_port=$app_port
						break
	...
	restore_docker() {
		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				read -e -p  "确认还原 Compose 项目 [$project_name] からパス [$original_path] ? (y/n): " confirm
				[[ "$confirm" != "y" ]] && read -e -p  "请输入新的还原路径: " original_path

				mkdir -p "$original_path"
				tar -xzf "$BACKUP_DIR/compose_project_${project_name}.tar.gz" -C "$original_path"
				echo -e "${gl_lv}Compose 项目 [$project_name] は次のように抽出されました。$original_path${gl_bai}"
...
linux_ldnmp() {
  while true; do
	...
	echo -e "${gl_huang}29.  ${gl_bai}ストリーム 4 層プロキシ転送${gl_huang}30.  ${gl_bai}自定义静态站点"
	echo -e "${gl_huang}------------------------"
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  b)
	  	...
	  	while true; do
			...
			case "$choice" in
			  [Yy])
				kj_ssh_read_host_port "请输入远端服务器IP:  " "ターゲット サーバーの SSH ポート [デフォルト 22]:" "22"
				local remote_ip="$KJ_SSH_HOST"
				local TARGET_PORT="$KJ_SSH_PORT"
	  ...
	  *)
		...
		if [ -f "$custom_app" ]; then
			...
			echo -e "${gl_hong}エラー: 番号が見つかりません${sub_choice} 的应用配置${gl_bai}"
		fi
		  ;;
	esac
	break_end
...
while true; do
	  ...
	  echo -e "${gl_kjlan}17. ${gl_bai}设置时区到上海           ${gl_kjlan}18. ${gl_bai}すべてのポートを開く${gl_kjlan}51. ${gl_bai}カスタム命令"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  echo -e "${gl_kjlan}0.  ${gl_bai}返回主菜单"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
	  ...
	  case $sub_choice in
		  ...
		  4)
			  clear
			  send_stats "备份集群"
			  echo -e "请将 ${gl_huang}/root/cluster/servers.py${gl_bai}ファイルをダウンロードしてバックアップを完了してください。"
			  break_end
			  ;;
		  ...
		  5)
			  ...
			  echo -e "をアップロードしてください${gl_huang}servers.py${gl_bai} 文件到 ${gl_huang}/root/cluster/${gl_bai}復元完了！"
			  break_end
			  ;;
