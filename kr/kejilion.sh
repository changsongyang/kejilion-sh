		...
		1)
			...
			while true; do
				...
				if ss -tuln | grep -q ":$app_port "; then
					echo -e "${gl_hong}错误: ${gl_bai}포트$app_port 已被占用，请更换一个端口"
		...
		case $choice in
			1)
				...
				while true; do
					...
					if ss -tuln | grep -q ":$app_port "; then
						echo -e "${gl_hong}错误: ${gl_bai}포트$app_port이미 사용 중입니다. 포트를 변경하세요."
						send_stats "애플리케이션 포트가 점유되어 있습니다."
					else
						local docker_port=$app_port
						break
...
ldnmp_install_status_one() {
   ...
   if docker inspect "php" &>/dev/null; then
	...
	echo -e "${gl_huang}힌트:${gl_bai}建站环境已安装。无需再次安装！"
	break_end
	linux_ldnmp
   fi
...
dd_xitong() {
		...
		dd_xitong_2() {
		  echo -e "重装后初始用户名: ${gl_huang}Administrator${gl_bai}  初始密码: ${gl_huang}Teddysun.com${gl_bai}초기 포트:${gl_huang}3389${gl_bai}"
...
SYSCTL
	...
	echo -e "${gl_lv}적용된${applied} 项参数${skipped:+，跳过 ${skipped}지원되지 않는 항목 매개변수}${gl_bai}"
...
LIMITS
	...
	echo -e "${gl_lv}메모리:${MEM_MB}MB | 拥塞算法: ${CC}| 대기줄:${QDISC}${gl_bai}"
}
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  if [ -n "$current_mode" ]; then
		  ...
		  echo -e "当前模式: ${gl_hui}최적화되지 않음${gl_bai}"
	  fi
...
linux_tools() {
  ...
  while true; do
	  ...
	  echo -e "${gl_kjlan}13.  ${gl_bai}ncdu 磁盘占用查看工具             ${gl_kjlan}14.  ${gl_bai}fzf 글로벌 검색 도구"
		...
		for c in "${TARGET_CONTAINERS[@]}"; do
			...
			if is_compose_container "$c"; then
				...
				if [[ -n "${PACKED_COMPOSE_PATHS[$project_dir]}" ]]; then
					echo -e "${gl_huang}Compose 项目 [$project_name] 이미 백업되어 있으니 반복 패키징은 생략하세요...${gl_bai}"
					continue
				fi
	...
	restore_docker() {
		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				echo -e "${gl_lv}프로젝트 작성 [$project_name] 已解压到: $original_path${gl_bai}"
		...
		for json in "$BACKUP_DIR"/*_inspect.json; do
			...
			if docker ps --format '{{.Names}}' | grep -q "^${container}$"; then
				echo -e "${gl_huang}컨테이너 [$container] 已在运行，跳过还原...${gl_bai}"
				continue
			fi
			...
			if docker ps -a --format '{{.Names}}' | grep -q "^${container}$"; then
				echo -e "${gl_huang}容器 [$container]가 있지만 실행 중이 아닙니다. 이전 컨테이너를 삭제하세요...${gl_bai}"
...
EOF
	...
	openclaw_memory_prepare_workspace_all() {
		...
		local agent_lines agent_id workspace
		agent_lines=$(openclaw_memory_list_agents)
		echo "检查并准备 $(printf '%s\n' "$agent_lines"| sed '/^\s*$/d' | 화장실 -l | tr -d ' ') 에이전트 작업공간"
		while IFS=$'\t' read -r agent_id workspace; do
			[ -z "$agent_id" ] && continue
...
EOF
	...
	openclaw_memory_view_file() {
		...
		read -e -p "시작 라인을 입력하십시오. (끝까지 기본값으로 설정하려면 Enter를 누르십시오.$default_lines 行）: " start_line
		...
		while true; do
			...
			case "$memory_choice" in
				...
				if [ "$confirm_step2" = "force" ]; then
					...
					echo "권장 사항: 안전한 재구축을 위해 재구축을 입력합니다(인덱스 데이터베이스를 먼저 백업)."
 ...
 print("%s. %s" % (idx, item.get("id","?"))); print("   workspace : %s" % item.get("workspace","-")); ident=(item.get("identityName") or "-") + ((" " + item.get("identityEmoji")) if item.get("identityEmoji") else ""); print("   identity  : %s" % ident.strip()); print("   model     : %s" % (item.get("model") or "-")); print("   bindings  : %s" % item.get("bindings",0)); print("   default   : %s" % ("yes" if item.get("isDefault") else "no"))' "$(openclaw_multiagent_agents_json)"
	...
	openclaw_multiagent_add_agent() {
		...
		if openclaw agents add "$agent_id" --workspace "$workspace"; then
			...
			read -e -p "상담원 성격 테마(예: 엄격함, 효율적)를 입력하세요." theme
			[ -z "$theme" ] && theme="어시스턴트"
			echo "에이전트 ID 구성 중..."
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}49.  ${color49}프로메테우스(컨테이너 모니터링)${gl_kjlan}50.  ${color50}补货监控工具"
	  echo -e "${gl_kjlan}-------------------------"
	  ...
	  echo -e "${gl_kjlan}73.  ${color73}LibreTV 개인 영화${gl_kjlan}74.  ${color74}MoonTV私有影视"
	  echo -e "${gl_kjlan}75.  ${color75}멜로디 음악 마법사${gl_kjlan}76.  ${color76}온라인 DOS 오래된 게임"
	  echo -e "${gl_kjlan}77.  ${color77}迅雷离线下载工具                    ${gl_kjlan}78.  ${color78}PandaWiki 지능형 문서 관리 시스템"
	  echo -e "${gl_kjlan}79.  ${color79}Beszel服务器监控                    ${gl_kjlan}80.  ${color80}링크워든 북마크 관리"
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}81.  ${color81}JitsiMeet 화상 회의${gl_kjlan}82.  ${color82}gpt-load高性能AI透明代理"
	  ...
	  echo -e "${gl_kjlan}87.  ${color87}SyncTV는 함께 영화를 볼 수 있는 훌륭한 도구입니다${gl_kjlan}88.  ${color88}Owncast自托管直播平台"
	  echo -e "${gl_kjlan}89.  ${color89}FileCodeBox 파일 익스프레스${gl_kjlan}90.  ${color90}매트릭스 분산형 채팅 프로토콜"
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}91.  ${color91}gitea私有代码仓库                   ${gl_kjlan}92.  ${color92}FileBrowser 파일 관리자"
	  ...
	  echo -e "${gl_kjlan}103. ${color103}Umami网站统计工具                   ${gl_kjlan}104. ${color104}스트림 4계층 프록시 전달 도구"
	  ...
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}111. ${color111}다중 형식 파일 변환 도구${gl_kjlan}112. ${color112}Lucky大内网穿透工具"
	  ...
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}b.   ${gl_bai}모든 애플리케이션 데이터 백업${gl_kjlan}r.   ${gl_bai}还原全部应用数据"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}返回主菜单"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
	...
	while true; do
		...
		case $choice in
			...
			2)
				...
				if ip link show "$nic" &>/dev/null; then
					ip link set "$nic" down && echo "✔ 네트워크 카드$nic 已禁用"
				else
					echo "✘ 网卡不存在"
				fi
				read -erp "按回车继续..."
...
echo "------------------------"
echo -e "${gl_kjlan}스크립트 공식 웹사이트:${gl_bai}https://kejilion.sh            ${gl_kjlan}GitHub地址: ${gl_bai}${gh_https_url}github.com/kejilion/sh${gl_bai}"
echo "------------------------"
echo ""
}
