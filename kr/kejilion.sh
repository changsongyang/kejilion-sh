		...
		1)
			...
			while true; do
				...
				if ss -tuln | grep -q ":$app_port "; then
					echo -e "${gl_hong}错误: ${gl_bai}端口 $app_port이미 사용 중입니다. 포트를 변경하세요."
					send_stats "应用端口已被占用"
				else
...
while true; do
	  ...
	  case $sub_choice in
		  ...
		  4)
			  clear
			  send_stats "备份集群"
			  echo -e "请将 ${gl_huang}/root/cluster/servers.py${gl_bai}파일을 다운로드하고 백업을 완료하세요!"
			  break_end
			  ;;
